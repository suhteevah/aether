...
- **multi-field enums across fn boundaries** (`bb05a81`) — passing a multi-field
  enum variant as a param/arg (`area(Shape::Rect(6,5))`) + matching it in the
...
- **probe sweep** (20-feature compositions): macro-in-array-literal, macro-arg-to-fn,
  ...
  with an array field passed BY VALUE to a method/free-fn (`self.d[i]`) — was a
  silent wrong answer (struct-by-value ABI collapsed the array to 1 slot); now the
  3 struct-by-value sites leaf-expand (witness `struct_array_by_value`). REMAINING
