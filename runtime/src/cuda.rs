...
struct CudaCtx {
    ...
    aether_mmvq_q6k_q8_1_single: CudaFunction,
    ...
    fused_q6_k_expert_matmul_seq1: CudaFunction,
...
// v4 -- memory-bandwidth-optimized Q4_K seq1 mat-vec (decode hot path).
//
// Baseline v3 achieves only ~10% of P100 HBM peak (~73 GB/s of 720): it is
// latency-starved, not compute-bound. vs v3, v4:
//   - NO shared mem + NO __syncthreads. v3 reloaded the 256-float activation
//     tile into shared every K-block with TWO syncs/block (2*n_blocks barriers),
//     serializing the 8 warps. v4 reads the activation straight from global; it
//     is tiny (n_in floats) and reused by every warp + every block in the grid,
//     so it stays L2-hot -- cheaper than the barrier stalls it replaces.
//   - scales computed INLINE per sub-block instead of precomputing d_eff[8]/
//     m_eff[8] register arrays -> lower register pressure -> higher occupancy
//     -> more concurrent warps -> more in-flight HBM requests to hide latency.
// Arithmetic + accumulation order are IDENTICAL to v3, so output is bit-exact
// (parity test asserts equality). 1 output row per warp; warps/block = launch
// blockDim.x/32 (tunable for occupancy without touching correctness).
extern "C" __global__ void fused_q4k_matmul_seq1_v4(
    const float*         __restrict__ a,
    const unsigned char* __restrict__ w,
    float*               __restrict__ out,
    int n, int n_blocks)
{
    ...
    const int ni = blockIdx.x * warps_per_block + (threadIdx.x >> 5);
    if (ni >= n) return;   // warp-uniform (same ni for all 32 lanes) -> shuffle safe

    const unsigned char* row = w + (size_t)ni * (size_t)n_blocks * 144;
    float acc_lo = 0.0f, acc_hi = 0.0f;

    for (int bi = 0; bi < n_blocks; bi++) {
        const unsigned char* base = row + (size_t)bi * 144;
        unsigned short d_bits    = ((unsigned short)base[1] << 8) | (unsigned short)base[0];
        unsigned short dmin_bits = ((unsigned short)base[3] << 8) | (unsigned short)base[2];
        ...
        const unsigned char* scales = base + 4;
        const unsigned char* qs     = base + 16;
        const float* ab = a + (size_t)bi * 256;

        #pragma unroll
        for (int i = 0; i < 4; i++) {
            const int sub_lo = i * 2;
            const int sub_hi = i * 2 + 1;
            ...
            const unsigned int byte = qs[i * 32 + lane];
            const float w_lo = d_lo * (float)(byte & 0xFu) - m_lo;
            const float w_hi = d_hi * (float)((byte >> 4) & 0xFu) - m_hi;
            acc_lo += ab[sub_lo * 32 + lane] * w_lo;
            acc_hi += ab[sub_hi * 32 + lane] * w_hi;
        }
    }
    float acc = acc_lo + acc_hi;
    #pragma unroll
...
// v5 -- multi-row-per-warp Q4_K seq1 mat-vec.
//
// Diagnosis: even lm_head (19008 CTAs -> full P100 occupancy) is stuck at ~10%
// of HBM peak, so the kernel is NOT occupancy/latency-hiding-limited -- each
// warp's single-row work is too small a dependency chain to keep enough HBM
// requests in flight. v5 gives each warp R=4 INDEPENDENT output rows -> 4
// independent weight-load streams + 4 accumulators per warp -> 4x the
// memory-level parallelism per warp without needing more occupancy. Keeps v3's
// shared activation tile (v4 proved global-activation reads are slower than the
// barriers they replace). Scales inlined per (row, sub-block) to hold register
// pressure down with R accumulators live.
extern "C" __global__ void fused_q4k_matmul_seq1_v5(
    const float*         __restrict__ a,
    const unsigned char* __restrict__ w,
    float*               __restrict__ out,
    int n, int n_blocks)
{
    ...
    const int warps_per_block = blockDim.x >> 5;
    const int row0 = (blockIdx.x * warps_per_block + warp) * R;

    float acc_lo[R], acc_hi[R];
    #pragma unroll
    for (int r = 0; r < R; r++) { acc_lo[r] = 0.0f; acc_hi[r] = 0.0f; }

    for (int bi = 0; bi < n_blocks; bi++) {
        // cooperative activation load (works for any blockDim).
        ...
        for (int r = 0; r < R; r++) {
            ...
            if (ni < n) {
                const unsigned char* base = w
                    + (size_t)ni * (size_t)n_blocks * 144 + (size_t)bi * 144;
                unsigned short d_bits    = ((unsigned short)base[1] << 8) | (unsigned short)base[0];
                unsigned short dmin_bits = ((unsigned short)base[3] << 8) | (unsigned short)base[2];
                const float d    = aether_f16_to_f32_dev(d_bits);
                const float dmin = aether_f16_to_f32_dev(dmin_bits);
                ...
                #pragma unroll
                for (int i = 0; i < 4; i++) {
                    const int sub_lo = i * 2, sub_hi = i * 2 + 1;
                    const float d_lo = d * (float)q4k_get_scale(sub_lo, scales);
                    const float m_lo = dmin * (float)q4k_get_min(sub_lo, scales);
                    const float d_hi = d * (float)q4k_get_scale(sub_hi, scales);
                    const float m_hi = dmin * (float)q4k_get_min(sub_hi, scales);
                    ...
                    const float w_lo = d_lo * (float)(byte & 0xFu) - m_lo;
                    const float w_hi = d_hi * (float)((byte >> 4) & 0xFu) - m_hi;
                    acc_lo[r] += a_tile[sub_lo * 32 + lane] * w_lo;
                    acc_hi[r] += a_tile[sub_hi * 32 + lane] * w_hi;
                }
            }
        }
        __syncthreads();
...
// unpack, no per-element float dequant). If this hits much higher GB/s than v3,
// the kernel is ALU/issue-bound on the dequant (-> integer-MAC restructure is
// the fix); if it's also ~73 GB/s, it's memory-access-pattern-bound (-> wider
// loads). Result is garbage; never wired into decode.
extern "C" __global__ void fused_q4k_membw_probe(
    const float*         __restrict__ a,
    const unsigned char* __restrict__ w,
    float*               __restrict__ out,
    int n, int n_blocks)
{
    ...
    const int lane = threadIdx.x & 31;
    const int warp = threadIdx.x >> 5;
    const int ni = blockIdx.x * 8 + warp;
    int isum = 0;
    float asum = 0.0f;
    for (int bi = 0; bi < n_blocks; bi++) {
        a_tile[threadIdx.x] = a[bi * 256 + threadIdx.x];
        ...
        if (ni < n) {
            const unsigned char* base = w + (size_t)ni * (size_t)n_blocks * 144 + (size_t)bi * 144;
            const unsigned char* qs = base + 16;
            // touch scales/d/dmin (read same bytes v3 reads) without unpacking.
            isum += base[0] + base[2] + base[4] + base[8] + base[12];
            // VECTORIZED weight read: each lane loads one uint (4 contiguous
            // bytes); 32 lanes cover the 128-byte qs in ONE 128-byte coalesced
            // transaction instead of 4 strided 32-byte byte-loads.
            const unsigned int* qs32 = (const unsigned int*)qs;
            unsigned int p = qs32[lane];
            isum += (int)((p & 0xFFu) + ((p >> 8) & 0xFFu) + ((p >> 16) & 0xFFu) + ((p >> 24) & 0xFFu));
            #pragma unroll
            for (int i = 0; i < 4; i++) {
                asum += a_tile[i * 64 + lane] + a_tile[i * 64 + 32 + lane];
            }
        }
        __syncthreads();
    }
    float acc = (float)isum + asum;
    #pragma unroll
    for (int o = 16; o > 0; o >>= 1) acc += __shfl_down_sync(0xFFFFFFFFu, acc, o);
    if (lane == 0 && ni < n) out[ni] = acc;
}
...
// FFN-section perf — FAITHFUL llama-MMVQ Q6_K vec_dot port for Phase 2c.
// Block layout (block_q6_K, 210 bytes): ql[128] | qh[64] | scales[16 i8] | d (half).
// Per-element value: 6-bit = (ql_nibble | (qh_2bit << 4)) - 32  (signed [-32,31]).
// Wrapper indexing (vecdotq.cuh:876-900): iqs ∈ 0..31, bq8_offset ∈ {0,1,4,5},
// scale_offset ∈ {0..3, 8..11}, vh_shift ∈ {0,2}.  vec_dot inner does QR6_K=2
// inner iterations (i=0,1) each covering 4 elements via dp4a with u[i].
extern "C" __device__ float aether_vec_dot_q6k_q8_1_p100(
    const unsigned char* __restrict__ w_block,    // 210-byte Q6_K super-block
    const signed char*   __restrict__ aq,
    const float*         __restrict__ ad,
    int q8_blk_base, int iqs)
{
    const unsigned char* ql_p     = w_block;
    const unsigned char* qh_p     = w_block + 128;
    const signed char*   scales_p = (const signed char*)(w_block + 192);
    unsigned short d_bits = ((unsigned short)w_block[209] << 8) | (unsigned short)w_block[208];
    float d = aether_f16_to_f32_dev(d_bits);

    // wrapper indexing
    int bq8_offset   = 2 * 2 * (iqs / 16) + (iqs % 16) / 8;   // QR6_K=2, QI6_K/2=16, /4=8
    int scale_offset = 8 * (iqs / 16) + (iqs % 16) / 4;
    int vh_shift     = 2 * ((iqs % 16) / 8);

    // Port of llama's get_int_b2 (vecdotq.cuh:18): reads via uint16_t* (2-byte
    // alignment), combines into int.  Required because block_q6_K is 210 bytes
    // — NOT 4-byte aligned across consecutive blocks (210%4 = 2), so reading as
    // (uint*) crashes with CUDA_ERROR_MISALIGNED_ADDRESS on half the blocks.
    // ql is 128 bytes inside the block: i32 ∈ 0..31 → covers all 32 ints.
    // qh is 64 bytes:                   i32 ∈ 0..15 → covers all 16 ints.
    const unsigned short* ql16 = (const unsigned short*)(ql_p + 4*iqs);
    unsigned int vl = ((unsigned int)ql16[0]) | (((unsigned int)ql16[1]) << 16);
    int vh_idx = 8 * (iqs / 16) + (iqs % 8);
    const unsigned short* qh16 = (const unsigned short*)(qh_p + 4*vh_idx);
    unsigned int vh = (((unsigned int)qh16[0]) | (((unsigned int)qh16[1]) << 16)) >> vh_shift;

    float sumf = 0.0f;
    #pragma unroll
    for (int i = 0; i < 2 /*QR6_K*/; i++) {
        int sc = (int)scales_p[scale_offset + 4*i];   // signed int8 scale
        int q8blk = q8_blk_base + bq8_offset + 2*i;
        float d8 = ad[q8blk];
        // u[i] = 4 int8 activations from Q8_1 block, at int offset (iqs % QI8_1) = iqs%8
        const unsigned int* u_p = (const unsigned int*)(aq + q8blk * 32) + (iqs % 8);
        unsigned int u = u_p[0];

        // vil = low 4 bits of each byte of (vl >> 4i)
        unsigned int vil = (vl >> (4*i)) & 0x0F0F0F0Fu;
        // vih = high 2 bits packed in positions 4-5 of each byte
        unsigned int vih = ((vh >> (4*i)) << 4) & 0x30303030u;
        unsigned int q_pack = vil | vih;   // each byte = unsigned 6-bit in [0,63]

        // dp4a-like: per byte, vi_byte = (q_byte - 32) as signed int8;
        // sumi += vi_byte * u_byte (signed * signed).
        const unsigned char* qb = (const unsigned char*)&q_pack;
        const signed char* ub = (const signed char*)&u;
        int sumi = 0;
        #pragma unroll
        for (int b = 0; b < 4; b++) {
            int vi = (int)qb[b] - 32;     // -32..31
            sumi += vi * (int)ub[b];
        }
        sumf += d8 * ((float)sumi * (float)sc);
    }
    return d * sumf;
}

// Faithful llama-MMVQ Q6_K single-output kernel.  vdr=1 → qi_over_vdr=32,
// blocks_per_iter=4.  Otherwise structurally identical to the Q4_K single
// kernel (block (32,4), one row/block, K-split + cross-warp reduce).
extern "C" __global__ void aether_mmvq_q6k_q8_1_single(
    const unsigned char* __restrict__ w,
    const signed char*   __restrict__ aq,
    const float*         __restrict__ ad,
    float*               __restrict__ out,
    int n_out, int blocks_per_row)
{
    ...
    const int qi_over_vdr     = 32;  // QI6_K / VDR_Q6_K_Q8_1_MMVQ = 32/1
    const int blocks_per_iter = 4;   // vdr*nwarps*warp_size/qi = 1*4*32/32

    int kbx_start = tid / qi_over_vdr;
    int iqs       = 1 * (tid % qi_over_vdr);

    const unsigned char* row_w = w + (size_t)row * blocks_per_row * 210;

    float tmp = 0.0f;
    for (int kbx = kbx_start; kbx < blocks_per_row; kbx += blocks_per_iter) {
        int q8_blk_base = kbx * 8;   // 8 Q8_1 blocks per super-block (256/32)
        tmp += aether_vec_dot_q6k_q8_1_p100(row_w + (size_t)kbx*210, aq, ad, q8_blk_base, iqs);
    }
...
// (d) qwen3moe — MoE expert-variant of Q6_K matmul.  Mirrors
// fused_q4k_expert_matmul_seq1 (one CTA per output row; 256 threads
// stride-loop the blocks) but with Q6_K dequant (210-byte 256-elem
// super-blocks).  Per-expert offset = `expert_offset_blocks * 210` bytes.
// Dequant layout matches ggml dequant_row_q6_K and the dense
// fused_q6k_matmul_seq1_v2.  Completes the expert dispatch table: every
// dense matmul dtype now has an expert variant.
extern "C" __global__ void fused_q6_k_expert_matmul_seq1(
    const float*         __restrict__ x,
    const unsigned char* __restrict__ w_base,
    float*               __restrict__ y,
    int n_out, int blocks_per_row, int expert_offset_blocks)
{
    ...
    if (o >= n_out) return;
    int tid = threadIdx.x;
    size_t exp_off_bytes = (size_t)expert_offset_blocks * 210;
    size_t row_off_bytes = (size_t)o * (size_t)blocks_per_row * 210;
    const unsigned char* w_row = w_base + exp_off_bytes + row_off_bytes;
    float acc = 0.0f;
    for (int b = tid; b < blocks_per_row; b += blockDim.x) {
        const unsigned char* blk = w_row + (size_t)b * 210;
        const unsigned char* ql = blk;            // 128 bytes (low 4 bits)
        const unsigned char* qh = blk + 128;      // 64 bytes (high 2 bits)
        const signed char*   sc = (const signed char*)(blk + 192); // 16 scales
        unsigned short d_bits = ((unsigned short)blk[209] << 8) | (unsigned short)blk[208];
        float d = aether_f16_to_f32_dev(d_bits);
        const float* x_blk = x + (size_t)b * 256;
        float blk_acc = 0.0f;
        // Two 128-element halves: ql+=64, qh+=32, sc+=8 per half.
        for (int n = 0; n < 2; n++) {
            int ql_off = n * 64;
            int qh_off = n * 32;
            int sc_off = n * 8;
            ...
            for (int l = 0; l < 32; l++) {
                int is = l >> 4;   // 0 or 1
                unsigned char qhb = qh[qh_off + l];
                int q1 = (int)((ql[ql_off + l]      & 0xFu) | (((qhb >> 0) & 3u) << 4)) - 32;
                int q2 = (int)((ql[ql_off + l + 32] & 0xFu) | (((qhb >> 2) & 3u) << 4)) - 32;
                int q3 = (int)((ql[ql_off + l]      >>  4)  | (((qhb >> 4) & 3u) << 4)) - 32;
                int q4 = (int)((ql[ql_off + l + 32] >>  4)  | (((qhb >> 6) & 3u) << 4)) - 32;
                blk_acc += x_blk[y_off + l +  0] * (d * (float)sc[sc_off + is + 0] * (float)q1);
                blk_acc += x_blk[y_off + l + 32] * (d * (float)sc[sc_off + is + 2] * (float)q2);
                blk_acc += x_blk[y_off + l + 64] * (d * (float)sc[sc_off + is + 4] * (float)q3);
                blk_acc += x_blk[y_off + l + 96] * (d * (float)sc[sc_off + is + 6] * (float)q4);
            }
        }
        acc += blk_acc;
    }
...
// matt-voice FR-17.14-extra-qlora-bwd — standalone IQ3_XXS dequant to f32.
//
// roadmap: P18
//
// Needed by aether_op_quant_matmul_backward_lhs_f32_cuda (dt=18) so the
// frozen-base QLoRA backward (dx = Wᵀ·dy) works against IQ3_XXS-quantised
// weights — the target base quant for 70B QLoRA training (Llama-3.3-70B
// IQ3_XXS ≈ 27.5 GB). The unpacking is extracted verbatim from
// fused_iq3_xxs_matmul_seq1; here it writes the dequantised W instead of
// accumulating into a dot product. One thread per output element.
//
// IQ3_XXS block layout (98 bytes / 256 elems): see fused_iq3_xxs_matmul_seq1.
// Output index `qi` within a block maps to (ib32, l, j, half) via:
//   ib32 = qi / 32 ; rem = qi % 32 ; l = rem / 8 ; jj = rem % 8
//   half = jj / 4 (0 = grid1/low-sign, 1 = grid2/high-sign) ; j = jj % 4
extern "C" __global__ void dequant_iq3_xxs(
    const unsigned char* __restrict__ blocks,
    float*               __restrict__ out,
    int n_blocks)
{
    ...
    int l    = rem >> 3;         // 0..3
    int jj   = rem & 7;          // 0..7
    int half = jj >> 2;          // 0 -> grid1/low sign, 1 -> grid2/high sign
    int j    = jj & 3;           // 0..3

    const unsigned char* base = blocks + (size_t)bi * 98;
    unsigned short d_bits = ((unsigned short)base[1] << 8) | (unsigned short)base[0];
    float d = aether_f16_to_f32_dev(d_bits);
    const unsigned char* qs  = base + 2;
    const unsigned char* sas = base + 2 + 64;
    ...
    unsigned int sign_idx = (aux32 >> (7 * l)) & 127u;
    unsigned int signs    = (unsigned int)ksigns[sign_idx];
    unsigned int grid = iq3xxs_grid[qs[8*ib32 + 2*l + half]];
    unsigned int q = (grid >> (8 * j)) & 0xFFu;
    int sign_bit = j + (half ? 4 : 0);
    float s = (signs & (1u << sign_bit)) ? -1.0f : 1.0f;

    out[idx] = db * (float)q * s;
}
...
fn ctx() -> &'static CudaCtx {
    CTX.get_or_init(|| {
        ...
        let aether_mmvq_q6k_q8_1_single = device.get_func("aether_kernels", "aether_mmvq_q6k_q8_1_single").unwrap();
        ...
        let fused_q6_k_expert_matmul_seq1 = device.get_func("aether_kernels", "fused_q6_k_expert_matmul_seq1").unwrap();
...
#[no_mangle] pub extern "C" fn aether_op_quant_matmul_backward_lhs_f32_cuda(
    ...
    let drc = match dt {
        ...
        // matt-voice FR-17.14-extra-qlora-bwd — IQ3_XXS (dt=18) is the target
        // base quant for 70B QLoRA training (Llama-3.3-70B IQ3_XXS ≈ 27.5 GB).
        // roadmap: P18
        18 => aether_op_dequant_iq3_xxs_f32_cuda(w, w_f32, n_blocks),
...
/// (d) qwen3moe — Q6_K MoE expert matmul.  CTA-per-output-row, 256 threads
/// (8 warps) block-stride the super-blocks, then warp + cross-warp reduce —
/// same launch geometry as the iq3_s/q4k expert wrappers.
#[no_mangle] pub extern "C" fn aether_op_fused_q6_k_expert_matmul_seq1_cuda(
    x: i64, w_base: i64, y: i64,
    n_out: c_int, blocks_per_row: c_int, expert_idx: c_int,
) -> c_int {
    let (Some(ix), Some(iy)) = (handle_to_idx(x), handle_to_idx(y)) else { return -1; };
    ...
    unsafe {
        let xv = &*x_p; let yv = &mut *y_p; let wv = &*w_p;
        ctx().fused_q6_k_expert_matmul_seq1.clone()
            .launch(cfg, (xv, wv, yv, n_out, blocks_per_row, expert_offset_blocks))
            .expect("launch fused_q6_k_expert_matmul_seq1");
    }
    0
}
...
/// FFN-section perf — FAITHFUL llama-MMVQ Q6_K single-output (for the k/v
/// matmuls on Q6_K layers of Qwen2.5-7B Q4_K_M, and any other Q6_K weight).
#[no_mangle] pub extern "C" fn aether_op_mmvq_q6k_q8_1_single_cuda(
    w_dev_u8: i64,
    aq_dev_u8: i64, ad_dev_f32: i64,
    out_dev_f32: i64,
    n_out: c_int, blocks_per_row: c_int,
    ...
    unsafe {
        let wv = &*w_p; let aqv = &*aq_p; let adv = &*ad_p; let ov = &mut *o_p;
        ctx().aether_mmvq_q6k_q8_1_single.clone()
            .launch(cfg, (wv, aqv, adv, ov, n_out, blocks_per_row))
            .expect("launch aether_mmvq_q6k_q8_1_single");
    }
    0
}
...
/// v4 Q4_K seq1 mat-vec — bandwidth-optimized (no shared/no sync/inline scales).
/// Bit-identical output to v3. `warps_per_block` (= outputs per CTA) is tunable
/// via `AETHER_Q4K_V4_WPB` (1..=32, default 8) to sweep occupancy without a
/// rebuild; shared_mem_bytes is 0 (kernel uses none).
#[no_mangle] pub extern "C" fn aether_op_fused_q4k_matmul_seq1_v4_cuda(
    a_dev_f32: i64, w_dev_u8: i64, out_dev_f32: i64,
    n: c_int, n_blocks: c_int,
) -> c_int {
    let Some(i_a) = handle_to_idx(a_dev_f32) else { return -1; };
    ...
    let Some(i_o) = handle_to_idx(out_dev_f32) else { return -1; };
    if n <= 0 || n_blocks <= 0 { return -1; }
    let wpb: u32 = std::env::var("AETHER_Q4K_V4_WPB").ok()
    ...
    let cfg = LaunchConfig {
        grid_dim:  (grid_x, 1, 1),
        block_dim: (wpb * 32, 1, 1),
        shared_mem_bytes: 0,
    };
    unsafe {
        let av = &*a_p; let wv = &*w_p; let ov = &mut *o_p;
        ctx().fused_q4k_matmul_seq1_v4.clone()
            .launch(cfg, (av, wv, ov, n, n_blocks))
            .expect("launch fused_q4k_matmul_seq1_v4");
    }
    0
}

/// v5 Q4_K seq1 mat-vec — multi-row-per-warp (R=4 rows/warp) for memory-level
/// parallelism. Bit-identical per-row output to v3. `warps_per_block` tunable
/// via `AETHER_Q4K_V5_WPB` (1..=16, default 4); each warp owns 4 rows so a CTA
/// covers warps_per_block*4 output rows.
#[no_mangle] pub extern "C" fn aether_op_fused_q4k_matmul_seq1_v5_cuda(
    a_dev_f32: i64, w_dev_u8: i64, out_dev_f32: i64,
    n: c_int, n_blocks: c_int,
) -> c_int {
    let Some(i_a) = handle_to_idx(a_dev_f32) else { return -1; };
    ...
    let Some(i_o) = handle_to_idx(out_dev_f32) else { return -1; };
    if n <= 0 || n_blocks <= 0 { return -1; }
    let wpb: u32 = std::env::var("AETHER_Q4K_V5_WPB").ok()
        .and_then(|v| v.parse().ok())
    ...
    let w_p = bs_u8[i_w].as_ref().unwrap() as *const CudaSlice<u8>;
    let o_p = bs[i_o].as_mut().unwrap() as *mut CudaSlice<f32>;
    let rows_per_cta = wpb * R;
    let grid_x = ((n as u32) + rows_per_cta - 1) / rows_per_cta;
    let cfg = LaunchConfig {
        grid_dim:  (grid_x, 1, 1),
        block_dim: (wpb * 32, 1, 1),
        shared_mem_bytes: 0,
    ...
    unsafe {
        let av = &*a_p; let wv = &*w_p; let ov = &mut *o_p;
        ctx().fused_q4k_matmul_seq1_v5.clone()
            .launch(cfg, (av, wv, ov, n, n_blocks))
            .expect("launch fused_q4k_matmul_seq1_v5");
    }
    0
}

/// DIAGNOSTIC: memory-ceiling probe (minimal ALU, same byte reads as v3).
/// Output is meaningless — perf only. Same launch as v3 (8 rows/CTA).
#[no_mangle] pub extern "C" fn aether_op_fused_q4k_membw_probe_cuda(
    a_dev_f32: i64, w_dev_u8: i64, out_dev_f32: i64,
    n: c_int, n_blocks: c_int,
) -> c_int {
    let Some(i_a) = handle_to_idx(a_dev_f32) else { return -1; };
    ...
    let w_p = bs_u8[i_w].as_ref().unwrap() as *const CudaSlice<u8>;
    let o_p = bs[i_o].as_mut().unwrap() as *mut CudaSlice<f32>;
    let grid_x = ((n as u32) + 7) / 8;
    let cfg = LaunchConfig {
        grid_dim: (grid_x, 1, 1), block_dim: (256, 1, 1), shared_mem_bytes: 0,
    };
    unsafe {
        let av = &*a_p; let wv = &*w_p; let ov = &mut *o_p;
...
/// roadmap: P18
///
/// W is a u8 device buffer of `n_blocks` IQ3_XXS super-blocks (98 bytes
/// each, 256 elems each); `out_f32` receives `n_blocks * 256` dequantised
/// f32 values in natural order. Mirrors aether_op_dequant_q{4,6}_k* so the
/// QLoRA backward op can route dt=18 the same way it routes Q4_K / Q6_K.
#[no_mangle] pub extern "C" fn aether_op_dequant_iq3_xxs_f32_cuda(
    blocks_u8: i64, out_f32: i64, n_blocks: c_int,
) -> c_int {
    let Some(i_blk) = handle_to_u8_idx(blocks_u8) else { return -1; };
    let Some(i_out) = handle_to_idx(out_f32) else { return -1; };
    ...
    unsafe {
        let bv = &*b_p; let ov = &mut *o_p;
        ctx().dequant_iq3_xxs_gpu.clone()
            .launch(cfg, (bv, ov, n_blocks))
            .expect("launch dequant_iq3_xxs");
    }
    0
}
